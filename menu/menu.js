 ...
 * 
 * Includes code from boskokukanjac at https://www.jqueryscript.net/menu/Create-A-Windows-Like-Multi-level-Dropdown-Menu-with-jQuery.html under MIT license
 ...
 */
 ...
 var MainMenu = function () {
    ...
    this.init = function (p) {
        ...
        // Show the sub-menu when mouse hovers for more than the time of timeout
        document.querySelectorAll('ul.main-menu > li > ul li').forEach(function (element){
            element.addEventListener("mouseenter", function(e){
                // Hide all other opened submenus in same level of this item
                if (this.classList.contains('separator')) return;
        ...
        document.querySelectorAll('ul.main-menu > li > ul li').forEach(function (element) {
            if (element.querySelectorAll('ul').length > 0) {
                element.classList.add(settings.submenuClass);
				element.innerHTML += '<i class="fas fa-play menu-arrow"></i>';
            }
        });

        document.querySelectorAll('ul.main-menu li.' + settings.disabledClass).forEach(function (element){
